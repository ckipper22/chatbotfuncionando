...
// CORREÇÃO DE TIPAGEM GEMINI FINALIZADA: result.response.text.trim() -> result.response.text().trim()
// ====================================================================================
...
const hasCustomSearchConfig = !!(CUSTOM_SEARCH_API_KEY && CUSTOM_SEARCH_CX); // Nova flag

// =========================================================================
// FUNÇÕES DE UTILIDADE (MOCK: SUBSTITUA PELA SUA IMPLEMENTAÇÃO REAL)
// =========================================================================

// Função simulada (MOCK) para enviar mensagens ao WhatsApp
...
async function processarMensagemCompleta(from: string, whatsappPhoneId: string, messageText: string) {
    ...
    if (hasGeminiConfig) {
        try {
            ...
            // Modelo configurado no seu prompt: gemini-2.5-flash
            const model = ai.getGenerativeModel({ model: "gemini-2.5-flash" }); 

            const result = await model.generateContent(messageText);
            
            // CORREÇÃO FINAL APLICADA AQUI: result.response.text()
